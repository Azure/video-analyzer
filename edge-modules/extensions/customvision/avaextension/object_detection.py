...
class ObjectDetection(object):
    ...
    def postprocess(self, prediction_outputs):
        ...
        # We modified the lines below for AVA:
        return [{
            "type": "entity",
            "entity": {
                "tag": {
                    "value": self.labels[selected_classes[i]],
                    "confidence": round(float(selected_probs[i]), 8)
                },
                "box": {
                    "l": round(float(selected_boxes[i][0]), 8),
                    "t": round(float(selected_boxes[i][1]), 8),
                    "w": round(float(selected_boxes[i][2]), 8),
                    "h": round(float(selected_boxes[i][3]), 8)
                }
            }
