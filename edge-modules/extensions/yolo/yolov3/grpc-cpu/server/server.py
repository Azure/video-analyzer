...
def Main():
    try:
        ...
        objectTag = ap.GetObjectTag()
        logging.info('gRPC server port: {0}'.format(grpcServerPort, objectTag))
        logging.info('Inference confidence: {0}'.format(inferenceConfidence))
        logging.info('Object tag: {0}'.format(objectTag))

        # create gRPC server and start running
        server = grpc.server(futures.ThreadPoolExecutor(max_workers=3))
        extension_pb2_grpc.add_MediaGraphExtensionServicer_to_server(InferenceEngine(inferenceConfidence, objectTag), server)
